...
export default function Students() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editData) {
        const { id, ...rest } = form;
        await updateDocument('students', editData.id, rest);
        showNotification('ছাত্রের তথ্য আপডেট হয়েছে ✅');
      } else {
        const studentId = `DT${Date.now()}`;
        await addDocument('students', { ...form, studentId });
        showNotification('নতুন ছাত্র যোগ হয়েছে ✅');
      }
      setShowModal(false);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      showNotification('ছাত্রের তথ্য মুছে ফেলা হয়েছে ✅');
    } catch (err) {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {students.length === 0 && (
        <div className="card text-center py-16">
          <MdSchool size={48} className="text-gray-300 mx-auto mb-3" />
          <p className="text-gray-400 text-lg">এখনো কোনো ছাত্র যোগ করা হয়নি</p>
          <button onClick={openAdd} className="btn-primary mt-4"><MdAdd size={18} /> প্রথম ছাত্র যোগ করুন</button>
        </div>
      )}
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                {[
                  ...
                  ['ভর্তির তারিখ', viewStudent.admissionDate],
